...
class dataset():
    def __init__(self,file_path):
        ...
        train_index=int(self.row*0.9)

        
        self.train_data=self.data.iloc[0:train_index].to_numpy()
        max=np.max(self.train_data,axis=0)
        min=np.min(self.train_data,axis=0)
        self.train_data=(self.train_data-min)/(max-min)


        self.test_data=self.data.iloc[train_index:].to_numpy()
        self.test_data=(self.test_data-min)/(max-min)


        self.train_label=self.y.iloc[0:train_index].to_numpy()
        self.test_label=self.y.iloc[train_index:].to_numpy()
